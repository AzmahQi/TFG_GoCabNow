...
export async function updateContactNumber(userId, newContactNumber) {
  try {
    ...
    if (!existingUser) {
      return {id:500, message:"User not found for the specified user ID"};
    }
    ...
    if (
      ...
      if (userWithNewContactNumber) {
        return {id:500, message:"Contact number already exists in the system"};
      }
    }

    if(!regex.test(newContactNumber)) {
      return {id:500, message:"Contact number doesn't have the correct format +34 plus 9 numbers"};
    }
...
export async function updateUserProfile(userId, updatedProfile) {
  try {
    ...
    if (updateNumber?.id != 500 && updatedProfileData?.id != 500) {
      return {
        id: 1,
        message: "Updated successfully!",
      };
    } else {
      return { id: 2, message: "Couldn't update it. Please try with another number." };
    }
...
export async function createUser(userData) {
  ...
  if (existingUserByNumber) {
    ...
    if (existingUserByNumber.email !== null) {
      ...
      const existingProfile = await getProfileByUserId(existingUserByNumber.id)
